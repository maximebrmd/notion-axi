...
Page bodies are markdown (via the Notion API), so `--content`, `--append`, and `--replace` all take markdown. For `db` commands, `<id>` may be a database **or** a data-source id — a database resolves to its first data source automatically (use `--source <id>` to target a specific one).

### Commands

| Command  | Description                                                  |
| -------- | ------------------------------------------------------------ |
| `search` | Search pages & databases shared with the integration         |
| `page`   | Pages — `view`, `create`, `update` (append/replace markdown) |
| `db`     | Databases — `view` (schema), `query` (rows)                  |
