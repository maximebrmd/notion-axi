...
Use `notion-axi` for Notion: search; read pages & databases; create, update (body + `--set`), archive, and move pages; create & edit databases; read/add comments; `whoami`; upload files; inspect/delete blocks; or call any REST endpoint directly with `api`.
